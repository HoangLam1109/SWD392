...
const getGameId = (game: LibraryGameApiGame): string =>
  game._id ?? game.id ?? "";
...
export const libraryGameService = {
  ...
  async getMyLibraryGames(userId: string): Promise<LibraryGameView[]> {
    ...
    const games = normalizeGames(
      gamesResponse.data as
        | { data?: LibraryGameApiGame[] }
        | LibraryGameApiGame[],
    );
  ...
  async getHighestScoreByUser(): Promise<LibraryGameRecord> {
    const response = await apiClient.get("/library-game/user/highest-score");
    return response.data as LibraryGameRecord;
  },

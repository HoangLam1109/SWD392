...
export class GameSessionService {
	...
	async end(id: string, endGameSessionDto: EndGameSessionDto): Promise<GameSessionDocument> {
		const existingSession = await this.findOne(id);
	...
	private async syncLibraryGameTotalPlaytime(libraryGameId: string): Promise<void> {
		const totalPlaytime =
